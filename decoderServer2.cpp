				...
				)
{
  ...
  for  (int textNum = 0;  textNum < NUM_TEXTS_PER_THREAD;  textNum++)
  {
    ...
    if (turn!=threadNumber){
      pthread_cond_wait(&threadCond[turn], &lock);}
    numBytes=read(messagePtr->getClientFd(), text, BUFFER_LEN);
    printf("Thread %d: cipher text: %s",threadNumber,text);

    //  YOUR CODE HERE                                                                                                             
    write(toDecodeFd, text, numBytes);
    numBytes=read(toParentFd, text, numBytes);
    text[numBytes]='\0';
